...
;; For properly encoding of filenames
...
(defun TeX-start-shell ()
  ...
  (save-excursion
    ...
    (with-current-buffer (make-shell "TeX-shell" nil nil "-v"))
...
(defun chooseA ()
  ...
  (if billomove (progn (setq billomove nil)(forward-line -1)) (insert-char 65 1))
)
...
(defun chooseB ()
  ...
  (interactive)
  (if billomove (progn (setq billomove nil)(forward-line 1)) (insert-char 66 1))
)
...
;  (interactive)
;  (dired "/-:bkna@computer.tukorea.ac.kr#22000:/home/professor/bkna/"))

;;; M-x tramp-shell (below) works for ssh shells too.
;(defun tramp-shell ()
;  "Run a shell in tramp like /docker:user@container:/ or /ssh:user@host#port:/"
;  (interactive)
;  (let* ((dir (read-string "tramp directory: " nil))
